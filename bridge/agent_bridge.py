...
class AgentBridge:
    ...
    def get_agent(self, session_id: str = None, agent_id: str = None,
                  user_id: Optional[int] = None,
                  team_ids: Optional[List[int]] = None) -> Optional[Agent]:
        """
        ...
        with self._agents_lock:
            ...
            elif user_id is not None and getattr(agent, "user_id", None) != user_id:
                logger.info(
                    f"[AgentBridge] Session {session_id} owner changed "
                    f"({getattr(agent, 'user_id', None)} -> {user_id}), re-initializing agent"
                )
                agent = self.initializer.initialize_agent(
                    session_id=session_id, agent_id=resolved_agent_id, user_id=user_id
                )
                self._agent_instances[key] = agent
                if resolved_agent_id == self.agent_registry.default_agent_id:
                    self.agents[session_id] = agent
    ...
    def agent_reply(self, query: str, context: Context = None, 
        ...
        try:
            ...
            agent = self.get_agent(
                session_id=session_id, agent_id=resolved_agent_id,
                user_id=ctx_user_id, team_ids=ctx_team_ids
            )
            if not agent:
