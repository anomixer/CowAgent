...
CowAgent 支援多使用者認證與 RBAC（角色權限控制），適合團隊部署。

### 啟用方式

在 `config.json` 中加入 `"multi_user": true`：
...
> ⚠️ 啟用後 `web_password` 會自動失效。**第一個註冊的使用者**自動成為**主管理員（id=1）**，其他管理員無法刪除或降級主管理員。

### 使用者管理

| 角色 | 權限範圍 |
|------|----------|
| **admin** | 完整權限：管理使用者、團隊、全域設定、模型設定、知識庫 |
| **user** | 個人對話、個人知識庫、所屬團隊空間 |

- **第一人註冊**自動成為 admin
- **管理員後台**：Web 控制台 → 👥 使用者管理
- **修改密碼**：個人設定頁面
- **主管理員（id=1）**受保護，其他管理員無法刪除或降級

### 資料庫

使用者帳號與 session 儲存在工作區的 `conversations.db`（`~/cow/sessions/`）：

| 資料表 | 內容 |
|--------|------|
| `mu_users` | 註冊使用者（使用者名稱、密碼雜湊、角色） |
| `mu_sessions` | 登入 session（7 天有效） |
| `mu_teams` | 團隊（含共享提示詞） |
| `mu_team_members` | 團隊成員與角色 |
| `mu_kb_shares` | 知識庫分享記錄 |
...
# 重新啟動後，第一個註冊的人會變成 admin
```

### 團隊管理與三層 Prompt 繼承

管理員可以建立團隊、加入成員，並設定**團隊提示詞**。系統支援 **三層 System Prompt 繼承**：
- **全域提示詞 (Global Prompt)**：由 Admin 於 Web 控制台統一設定，對所有使用者的對話生效。
- **團隊提示詞 (Team Prompt)**：團隊共享規範，由團隊成員自動繼承。
- **個人提示詞 (User Prompt)**：個人在設定頁微調，具備最高權重覆蓋能力。

> 💡 Prompt 的注入完全在**純記憶體 (In-Memory)** 中併發執行，絕不修改磁碟檔案，支援高併發多人同時登入與獨立對話。

### 知識庫隔離與分享

每個使用者的知識庫檔案隔離在 `knowledge/users/{user_id}/` 目錄下。使用者可以透過知識庫 → 分享面板，選擇將知識庫分享給其他使用者。
