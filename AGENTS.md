...
#### ✅ Phase 3.5 — 團聊 IM 即時訊息系統
團聊頁面 UI (左側導航 對話/團聊)、跨成員即時同步、@AI 標記機制、個人 Session 嚴格隔離
...
## 14. Phase 3.5 — 團聊 IM 即時訊息系統

### 架構概覽

```
團聊頁面 (currentView = 'team-chat')
    │
    ├── 左側導航：對話 (個人) / 團聊 (team) 二分頁籤
    │       └── switchTeamSession(teamId, teamName)
    │               ├── _saveSessionId(`team_session_${teamId}`)
    │               ├── messagesDiv.innerHTML = ''
    │               ├── _teamRenderedSeqs.clear()  ← 清除已渲染 seq 追蹤
    │               ├── _teamAiLoadingEl cleanup
    │               ├── loadHistory(1)              ← 初始歷史載入
    │               └── startPolling()              ← 啟動 1 秒輪詢循環
    │
    └── 1 秒輪詢循環 (startPolling → _syncTeamHistory)
            ├── fetch /api/history?session_id=team_session_X&page=1&page_size=50
            │       └── 回傳 { messages, active_request_id }
            │
            ├── AI 狀態同步 (observer pattern)
            │       ├── active_request_id 存在 && 自己沒有 activeStream → 顯示 loading spinner
            │       └── active_request_id 消失 && 有 _teamAiLoadingEl → 移除 spinner, loadHistory(1) 取完整回覆
            │
            └── 增量訊息追加 (Set-based dedup)
                    ├── 過濾 _seq NOT IN _teamRenderedSeqs
                    ├── user role: 若有 data-optimistic="1" 元素 → reconcile (補 seq, 移除 optimistic 標記)
                    │           否則 createUserMessageEl / createTeamMemberMessageEl
                    └── assistant role: 若 SSE 正在串流此 request → skip (避免與 SSE 競爭)
```

### Session 隔離設計

| Session 類型 | session_id 格式 | 用途 |
|---|---|---|
| **個人** | `session_{uuid}` | 個人 AI 對話 |
| **團聊** | `team_session_{team_id}` | 團隊共享聊天室 |

- `get_user_conversation_sessions` SQL 加 `AND session_id NOT LIKE 'team_%'` 過濾
- `list_sessions` 同樣過濾
- 前端 `_fetchSessionPage` 額外 `.filter(s => !s.session_id.startsWith('team_'))` 雙重防護
...
使用者輸入 "@AI 請幫我..."
    │
    ├── 前端: is_ai_tagged = true → 送出時帶 tag_ai: true
    ├── 前端: 顯示 AI loading spinner (loadingEl)
    ├── 後端: 走一般 Agent 流程 + SSE 串流
    └── 後端: active_session_requests[session_id] = request_id
                    │
                    └── 其他成員的 _syncTeamHistory 輪詢到 active_request_id
                            ├── 顯示 _teamAiLoadingEl (loading spinner)
                            └── AI 完畢後 → 移除 spinner → loadHistory(1) 取完整回覆
```

### Optimistic Message 去重機制

User1 發送訊息後，前端立即渲染 (optimistic)，同時在 DOM 元素上標記 `data-optimistic="1"`。1 秒後 `_syncTeamHistory` 收到 DB 訊息時：
- 找到 `[data-optimistic="1"]` → 補上 `data-seq`，移除 `optimistic` 標記 → **不創建重複元素**
- 找不到 → 正常 append (用於 user2/user3 看到 user1 的訊息)

SSE `done` 事件後，將 `user_seq` 和 `bot_seq` 加入 `_teamRenderedSeqs`，防止 AI 回覆也被重複 append。

### 關鍵變數與邏輯 (console.js)

| 變數 | 說明 |
|------|------|
| `_teamRenderedSeqs` | Set，記錄已渲染的所有 `_seq`，防止重複追加 |
| `_teamAiLoadingEl` | Observer 端的 AI thinking spinner DOM 元素（包含 `回答 <username> 中 ...` 標籤） |
| `_teamAiLoadingReqId` | 對應的 `active_request_id` |

### 關鍵後端變更 (web_channel.py)

| 位置 | 變更 |
|------|------|
| `WebChannel.__init__` | 新增 `active_session_requests: dict[str, str]` (session_id → request_id) |
| `post_message` | `is_team_session` 判斷，跳過 owner reassign；全域 `mu_user` 定義防 UnboundLocalError |
| `_drop_sse_request` | 同步清除 `active_session_requests` |
| `HistoryHandler.GET` | 回傳 `active_request_id` 欄位 |
| `_check_session_owner` | 非 team member 存取 team session → auto-join |
| `db.get_user_conversation_sessions` | SQL 過濾 `team_%` 團隊 session |
| `conversation_store.list_sessions` | SQL 過濾 `team_%` 團隊 session |

---

## 15. Phase 3.5 擴充 — 團隊工作區 (Team Workspace)、記憶與知識庫隔離

### 15.1 團聊空間中欄與團隊對話串層級 (Team Sub-threads)

* **中欄選單「團聊空間 (TEAM SPACES)」**：當切換至「團聊」或進入「團隊工作區」時，中間選單顯示層級卡片列表：
  ```text
  👥 團聊空間 (TEAM SPACES)
  ├── ⚙️ Team 1                 [3 人] [+]
  │     ├─ # 專案開發討論
  ...
  └── ⚙️ Team 2                 [3 人] [+]
        └─ # 系統架構討論
  ```
* **點擊團隊標題**：開啟該團隊的 **Team Workspace** 主頁（含有公告欄與 Prompt 編輯區）。
* **點擊 `+` 按鈕或團隊內按鈕**：可呼叫 `/api/teams/{id}/threads` 建立獨立的團隊子對話串（Session ID 格式：`team_{team_id}_{hash}`）。
* **點擊 `# 子對話串`**：直接切換至該子對話串並進行聊天。

### 15.2 團隊記憶與知識庫隔離機制

1. **團隊記憶隔離 (Team Memory)**：
   - 路徑結構：`memory/teams/{team_id}/`
   - `list_scoped_memories(user_id, role)` 服務介面：按 `個人記憶` 與 `團隊記憶`（依 Team 分組）回傳。
   - 權限約束：Admin 可查看所有團隊記憶，一般 Team Member 僅能存取自己所屬 Team 的記憶檔案。
2. **團隊知識庫樹狀圖 (Team Knowledge Base Tree)**：
   - 路徑結構：`knowledge/teams/{team_id}/`
   - `list_scoped_tree(user_id, role)` 服務介面：構建 `👤 個人知識庫` 與 `👥 團隊知識庫` 雙樹架構。
   - 前端繪製：在 `menu_knowledge` 呈現樹狀目錄，按 Team 隔離與分享。

### 15.3 CowAgent Pro 品牌與 UI 細節

* **多用戶品牌**：多用戶模式上線時，左上角 Logo、登入頁面與 Footer 版本號統一自動顯示為 **CowAgent Pro**。
* **AI 思考氣泡標籤 (`replying_to`)**：團聊對話中，AI 思考中的 `...`Spinner 氣泡上方會清晰標示 `回答 <username> 中 ...`，且訊息順序精確依照 `_seq` 時間戳排序，防止對話亂序。
