...
class WebChannel(ChatChannel):
    ...
    def __init__(self):
        ...
        # Upstream SSE rework (per-request stream state + lock).
        self.sse_streams = {}  # request_id -> SSEStreamState
        self._sse_streams_lock = threading.RLock()
        # Multi-user: which username is driving each session (used to build the
        # per-user prompt). request_id bookkeeping is kept for compatibility.
    ...
    def startup(self):
        ...
        if os.environ.get("COW_DESKTOP") != "1":
            # Try to open the browser.  On headless Linux / WSL the gio helper
            # often prints "Operation not supported" to stderr; redirect that
            # to os.devnull so it doesn't clutter the console.
            try:
